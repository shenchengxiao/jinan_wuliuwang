...
var Industry = {
	...
	confirm : function(){
		...
		indStr=indStr.substring(1)?indStr.substring(1):'请选择标签';
...
function IndustrySelect(){
	var dragHtml ='<div id="IndustryAlpha">';		//行业
		dragHtml+='		<dl id="IndustrySelected"><dt>已选标签：</dt><dd></dd></dl>';
		dragHtml+='		<div id="IndustryList"></div>';//行业列表
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择标签（您最多能选择5项）</b><span onclick="Industry.confirm()">确定</span>');
...
function IndustrySelect_2(){
	var dragHtml ='<div id="IndustryAlpha">';		//行业
		...
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择标签</b><span onclick="boxAlpha()">关闭</span>');
	$('#drag_con').html(dragHtml);
